...
//===========================================================================
// This one translates one key from 065 to 070 style conventios
...
int translate_key070(char *key)
{
    ...
    */
    ...
    const char **pp; char *d; int l, n, x, k, r, f;
    //char org[200]; strcpy(org, key);
    l = strlen(key);
    if (0 == l)
        return 0;
    ...
    for (pp = checklist, x = 0; x++ < 2; ++pp) {
        ...
        do {
            ...
            if ((f ? (d == key) : (d > key))
                && (f || d[-1] == '.' || d[-1] == '*')
                && 0 == memicmp(d, pp[0]+f, k)) {
                if (x == 1)
                    strcpy(key+l, ".appearance");
                else
                    strcpy(d, pp[1]);
                //dbg_printf("%s -> %s", org, key);
                return 1 + (x==2 && n == 0);
            }
        } while (++n, *(pp += x));
    }
    //dbg_printf("%s (no match)", org);
    return r;
}
...
//===========================================================================
// this one tries to satisfy a plugin that queries an 065 item
// from a style that has 070 syntax

struct lin_list *search_line_065(struct fil_list *fl, const char *key)
{
    char buff[MAX_KEYWORD_LENGTH]; int f; struct lin_list *tl; char *d;

    strlwr(strcpy_max(buff, key, sizeof buff));
...
FILE *create_rcfile(const char *path)
{
    ...
    if (NULL==(fp=fopen(path, rc_dos_eol ? "wt" : "wb")))
        BBMessageBox(MB_OK, NLS2("$BBError_WriteFile$",
            "Error: Could not open \"%s\" for writing."),
            string_empty_or_null(path));
    return fp;
}
...
ST struct fil_list *read_file(const char *filename)
{
    ...
    fl = rc_files, n = 4;
    while (fl) {
        struct fil_list *fl_this = fl;
         fl = fl->next;
        if (--n < 0)
            delete_fil_list(fl_this);
    }
    ...
    if (0 == strcmp(filename, stylePath(NULL))) {
        fl->is_style = true;
        check_style070(fl);
    }
...
{
    char buff[MAX_KEYWORD_LENGTH]; unsigned k;
    struct fil_list *fl;
    struct lin_list **slp, *sl, *tl, *nl;
    int dirty = 0;
